...
#define IN_ROUTERS_COUNT 26
#define OUT_ROUTERS_COUNT 16

int outRouters[OUT_ROUTERS_COUNT] = {0, 36, 37, 9, 38, 13, 16, 20, 18, 28, 21, 24, 26, 35, 34, 33};
...
main(int argc, char* argv[])
{
  ...
  ndnHelperNoCache.SetOldContentStore("ns3::ndn::cs::Freshness::Lru", "1000");
  ...
  // Add the bridge router prefixes
  ndnGlobalRoutingHelper.AddOrigins("/producer", nodes.Get(producerId));
  for (int i = 0; i < OUT_ROUTERS_COUNT; i++) {
    int routerIndex = NUM_OF_CONSUMERS + outRouters[i];
    std::stringstream sstm;
    sstm << "/router/" << routerIndex;
    std::string prefix = sstm.str();
    ndnGlobalRoutingHelper.AddOrigins(prefix, nodes.Get(routerIndex));
  }
